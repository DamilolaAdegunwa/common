...
//builder.Services.AddControllers();
#region ApiAuthorizationOptions code goes here
var security_key = builder.Configuration["AppSettings:SecretKey"];
//add code here
...
builder.Services.AddAuthorization(options =>
{
	...
	options.AddPolicy("CustomerSupportOnly", policy =>
	{
		policy.RequireAuthenticatedUser();
...
builder.Services.Configure<ApiAuthorizationOptions>(options =>
{
	...
	SecurityKey securityKeyObj = new RsaSecurityKey(rsaKey);

	// You can also specify additional properties for the SecurityKey if needed
	//securityKey.KeyId = "my-key-id";
	securityKeyObj.KeyId = security_key;

	// Use the securityKey in your code as needed
	options.SigningCredential = new SigningCredentials(/* Your signing key */securityKeyObj, SecurityAlgorithms.RsaSha256);
	#endregion
